...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!name) return { title: 'İsim Bulunamadı', robots: { index: false } }
  return getNamePageMetadata(name, 'tr')
}

export default async function TurkishNameDetailPage({ params }: Props) {
  const { slug } = await params
  const name = getNameBySlug(slug)
  if (!name) notFound()
  return <NameDetailArticle name={name} locale="tr" />
}
