...
export function NameDetailClient({ name }: { name: NameEntry }) {
  ...
  return (
    <article className="mx-auto max-w-3xl space-y-6">
      <Link href="/names" className="text-sm text-gold hover:text-gold-soft focus-ring rounded">Zur Uebersicht</Link>
      <section className="rounded-lg border border-gold/20 bg-surface p-6">
        ...
        <p className="mt-2 text-muted">Aussprache: {name.pronunciation}</p>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button className={learned ? 'btn-secondary' : 'btn-primary'} onClick={() => learned ? actions.unmarkLearned(name.id) : actions.markLearned(name.id, name.slug)}>
            {learned ? 'Als offen markieren' : 'Als gelernt markieren'}
          </button>
          <button className="btn-secondary" onClick={() => actions.toggleFavorite(name.id)}>
            {favorite ? 'Favorit entfernen' : 'Favorit'}
          </button>
        </div>
      </section>
      <InfoBlock title="Bedeutung" body={name.meanings[language]} />
      <InfoBlock title="Erklaerung" body={name.explanations[language]} />
      <InfoBlock title="Dua" body={name.duaUsage[language]} />
      {name.reflection && <InfoBlock title="Reflexion" body={name.reflection[language]} />}
      ...
      <div className="flex justify-between gap-4">
        <Link className="btn-secondary" href="/names">Zurueck</Link>
        <Link className="btn-primary" href={`/names/${next.slug}`}>Naechster Name</Link>
      </div>
    </article>
  )
}
