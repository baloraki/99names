  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-6 py-8 md:grid-cols-[1.1fr_0.9fr] md:items-center md:py-16">
        <div>
          <p className="text-sm uppercase tracking-[0.22em] text-gold">99 Names</p>
          <h1 className="mt-4 max-w-3xl text-4xl font-semibold leading-tight text-primary md:text-6xl">
            Die 99 Namen Allahs als ruhige Lernhilfe.
          </h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted">
            Suche, markiere deinen Fortschritt und lerne in deinem Tempo. Die Inhalte sind bewusst vorsichtig formuliert und muessen vor einer Veroeffentlichung fachkundig geprueft werden.
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link className="btn-primary" href="/names">Alle Namen ansehen</Link>
            <Link className="btn-secondary" href="/learn">Lernmodus starten</Link>
          </div>
        </div>
        <div className="rounded-lg border border-gold/25 bg-[radial-gradient(circle_at_top,rgba(214,178,94,0.18),rgba(22,22,22,0.96)_58%)] p-6">
          <p className="text-sm text-gold">Name des Tages</p>
      ...
      <div className="grid gap-4 md:grid-cols-[1fr_1fr]">
        ...
        <section className="rounded-lg border border-white/10 bg-surface p-4">
          <p className="text-sm text-muted">Lernplan</p>
          <h2 className="mt-2 text-xl font-semibold">{due ? 'Eine Lerneinheit ist faellig.' : 'Kein faelliger Hinweis.'}</h2>
          <p className="mt-2 text-sm leading-6 text-muted">
            Hinweise erscheinen nur in der geoeffneten App. Es gibt keine Push- oder Browser-Benachrichtigungen.
          </p>
        </section>
      </div>
    </div>
