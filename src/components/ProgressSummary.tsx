...
export function ProgressSummary({ progress, compact = false }: Props) {
  const total = 99
  const learned = progress.learnedIds.length
  const percent = getProgressPercentage(progress)

  return (
    <section className="rounded-lg border border-white/10 bg-white/[0.04] p-4" aria-label="Fortschritt">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm text-muted">Fortschritt</p>
        ...
        <div className="text-right">
          ...
          {!compact && <p className="text-sm text-muted">gelernt</p>}
        </div>
      </div>
